...
- [ ] 引入并注册 Tauri Updater 插件
- [ ] 配置 Updater 权限、公钥和 HTTPS 更新端点
- [ ] 继续使用官方 GitHub Releases 托管更新元数据和安装包
- [ ] 配置 Windows x64 NSIS Updater 构建产物
- [ ] 生成并发布每个更新安装包对应的 Updater 签名
- [ ] 建立签名私钥的本地或 CI Secret 管理方式，禁止提交仓库
- [ ] 应用初始化完成后非阻塞检查一次新版本
- [ ] 自动检查默认开启，并允许用户在设置页关闭
- [ ] 设置页提供不受自动检查开关影响的“检查更新”入口
- [ ] 发现更新时展示当前版本、新版本、发布时间和更新说明
- [ ] 用户确认后才下载和安装，不做静默或强制更新
- [ ] 展示检查中、无更新、可更新、下载中、安装中和失败状态
- [ ] 下载或安装过程中禁止重复触发
- [ ] 检查失败、离线或 GitHub 不可用时不影响核心本地功能
- [ ] 安装失败时保留当前版本，并提供重试和 GitHub Releases 手动下载入口
- [ ] 首版不做常驻轮询、灰度发布、强制更新或自动降级
