...
- 当前阶段：`v0.2.0` 游戏封面管理已发布，正在规划 `v0.2.1` 应用更新能力
...
- [ ] 继续使用官方 GitHub Releases 托管更新元数据和安装包
- [x] 配置 Windows x64 NSIS Updater 构建产物
- [ ] 生成并发布每个更新安装包对应的 Updater 签名
...
- [ ] 更新 Windows 发布 SOP，纳入安装包、Updater 签名和更新元数据检查
- [ ] 更新 README、需求文档和 `v0.2.1` 发布记录

#### 8.3.1 更新安全与发布约束

- [x] 应用只内置签名公钥，不包含签名私钥
- [x] 应用安装更新前必须通过 Updater 签名校验
- [ ] 更新元数据中的版本、下载地址和签名必须对应同一次正式发布
- [ ] 已公开版本的更新文件不得覆盖或替换，问题修复必须发布新版本
- [ ] 保留现有 SHA-256 发布文件和未进行 Windows 代码签名的风险说明
- [ ] 明确 Updater 签名与 Windows Authenticode 代码签名用途不同，不能混为一谈

### 8.4 v0.2.2：UI 基础组件统一

模块目标：在 `v0.2.1` 应用更新能力发布并稳定后，集中引入 Reka UI 重构共享交互组件；不在业务模块中长期保留自研复杂交互与 Reka UI 两套实现。完成后发布 `v0.2.2`。
