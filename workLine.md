...
- [ ] 在设置模块中实现联网封面开关、配置状态、API Key 的本地保存、连接测试和删除能力
- [ ] 新增 `OnlineCoverDialog`，负责一次搜索、当前匹配提示、匹配纠错、候选预览、加载状态和错误反馈
- [ ] 由统一封面选择入口协调本地文件选择、拖拽、剪贴板粘贴和按配置显隐的在线搜索

当前 Provider 已覆盖游戏搜索、候选解析、鉴权/超时/限流错误，以及保存时按 providerGameId + assetId 重新定位原图；尚未使用个人 API Key 做真实服务联调，也尚未接入 Tauri command。
