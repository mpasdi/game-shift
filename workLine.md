...
- [ ] 复用现有 `assets` 模块完成图片内容识别、尺寸校验、转码、本地缓存和旧文件清理
...
当前 Provider 已覆盖游戏搜索、候选解析、鉴权/超时/限流错误，以及保存时按 providerGameId + assetId 重新定位原图；设置模块已覆盖默认关闭的开关、四种配置状态、Key 本地保存、连接测试和删除。尚未使用个人 API Key 做真实服务联调，也尚未接入封面搜索 Tauri command。
