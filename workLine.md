...
当前联网封面纵向流程已覆盖游戏搜索、匹配纠错、候选选择，以及保存时按 providerGameId + assetId 重新定位原图、安全下载、图片校验、转码和本地缓存；设置模块已覆盖默认关闭的开关、四种配置状态、Key 本地保存、连接测试和删除。已使用真实服务完成搜索、匹配切换和候选展示验证，仍需补充保存后重启、异常回退等完整手动验收。
...
11. [ ] 完成联网封面手动验收
12. [ ] 更新 README、版本说明和 `v0.2.0` 发布记录
