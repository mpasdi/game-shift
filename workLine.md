...
- [ ] 更新三个位置的版本号。
- [ ] 更新 README 的当前状态、安装方式和版本信息。
- [ ] README 增加 Windows 系统要求与 GitHub Releases 下载说明。
- [ ] README 说明本地数据存储和卸载行为。
- [ ] README 记录当前已知限制。
- [ ] 整理本版本新增功能、修复内容和已知问题，供 GitHub Release 使用。
- [ ] 确认文档不再包含已经完成的“待打包”“待发布”等过时描述。
...
- [ ] 重复 `.exe` 路径无法重复导入。
- [ ] 中文、空格和较长路径可以正常处理。
- [ ] 编辑游戏名称、路径、工作目录和启动参数正常。
- [ ] 移除游戏只删除数据库记录，不删除本地文件。
- [ ] 收藏、取消收藏、重新收藏及收藏时间排序正常。
- [ ] 搜索在首页、全部游戏、收藏和最近游玩页面范围正确。
- [ ] 点击启动可以运行游戏。
- [ ] 启动后最近游玩时间和启动次数正确更新。
- [ ] 关闭并重新打开 Game Shift 后数据仍然存在。
- [ ] 设置页显示的版本号、标识和数据目录正确。
- [ ] 大屏、小屏和最小窗口尺寸下主要页面无明显错位或滚动异常。
