...
- [x] 封面入口提供“本地选择”和“在线搜索”，不在弹窗打开或名称变化时自动请求
...
- [ ] 后端仅接受 provider 和资源标识，不接受前端传入的任意下载 URL
- [ ] 在设置模块中保存用户自己的 API Key，不将固定密钥写入源码或打包进安装包
- [ ] 新增 `OnlineCoverDialog`，负责一次搜索、当前匹配提示、匹配纠错、候选预览、加载状态和错误反馈
- [ ] 由统一封面选择入口协调本地文件选择和在线搜索
...
5. [ ] 实现在线封面搜索与候选选择弹窗
6. [ ] 实现网络图片安全下载，并接入现有封面缓存链路
7. [ ] 将待选网络封面接入新增和编辑游戏的保存事务
8. [ ] 补齐无结果、断网、超时、限流、无效密钥和损坏图片提示
9. [ ] 补充 provider 解析、下载限制、封面选择状态和保存流程测试
10. [ ] 完成手动验收并更新 README、版本说明和 `v0.2.0` 发布记录
