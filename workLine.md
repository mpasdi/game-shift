...
- 当前阶段：`v0.2.0` 游戏封面管理已完成开发和验收，正在进行发布收尾
...
`v0.1.0` 已发布，`v0.2.0` 的可选联网封面纵向流程已完成开发、测试和手动验收，当前进入发布收尾：

1. [x] 在设置模块实现默认关闭的联网封面开关和配置状态
2. [x] 接入用户自备 SteamGridDB API Key 的本地保存、连接测试和删除能力
3. [x] 实现 SteamGridDB 游戏搜索和封面候选查询
4. [x] 实现在线封面搜索、匹配纠错和候选选择界面
5. [x] 实现网络封面安全下载并接入现有本地缓存链路
6. [x] 将待选网络封面接入新增和编辑游戏的保存流程
7. [x] 封装当前自研组件体系下的 `BaseSelect`，将匹配游戏切换改为单选下拉交互
8. [x] 补齐异常处理、测试和手动验收
...
### 9.3 v0.2.1：UI 基础组件统一

模块目标：在 `v0.2.0` 联网封面发布并稳定后，集中引入 Reka UI 重构共享交互组件；不在业务模块中长期保留自研复杂交互与 Reka UI 两套实现。完成后发布 `v0.2.1`。
